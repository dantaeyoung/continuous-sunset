...
function createTimeLocal(hr, min, sec) {
  var dst = true;
  if (dst) {
    hr -= 1
  }
  var mins = hr * 60 + min + sec/60.0
  return mins
}
...
function calculate_op() {
  ...
  var jday = createJD(op['y'], op['m'], op['d'])
  var tl = createTimeLocal(op['hr'], op['min'], op['sec'])
  var tz = op['tzoffset']
  var dst = op['dst']
  var total = jday + tl/1440.0 - tz/24.0
  var T = calcTimeJulianCent(total)
  var lat = parseFloat(op['lat'])
  var lng = parseFloat(op['lng'])
//  calcAzEl(1, T, tl, lat, lng, tz)
//  calcSolNoon(jday, lng, tz, dst)
  var rise = calcSunriseSetUTC(1, jday, lat, lng, tz, dst)
  var set  = calcSunriseSetUTC(0, jday, lat, lng, tz, dst)
...
keys.calculate_op = calculate_op;
